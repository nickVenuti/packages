# -*- coding: utf-8 -*-
"""
Created on Wed Dec 30 10:07:26 2015
...
def get_data_from_www(oType, matMonth, matYear, date):
    ...
    url = URL % (oType, matMonth, matYear, date)  # parameterizes the URL     
    req = Request(url)   
...
#    except:
#        raise Exception('Cause of the problem due to website: '+url)
#        return url
...
def data_collection(path):
    ...
    start = today - dt.timedelta(29)  # the last 31 days
        ...
        try:
            ...
            if series_name in store.keys():  # if data for that series exists
                ...
                if len(index_new - index_old) > 0:
                    store[series_name] = pd.concat((store[series_name],
            ...
            print str(month) + '/' + str(day) + ' not available'

    store.close()

#Get data
path = 'C:/Users/nmvenuti/Desktop/Background Reading/Python for Finance/Data/' 
